/**
 * Relaunch Strategy lead submission to bw-fub-proxy.
 */
(function (global) {
  'use strict';

  var FUB_PROXY_URL = 'https://bw-fub-proxy.scott-5f5.workers.dev';
  var LEAD_TAG = 'relaunch-inquiry';
  var LEAD_SOURCE = 'Relaunch Strategy Page';
  var PAGE_HOST = 'relaunch.brightworkrealty.com';
  ...
  function buildPayload(firstName, lastName, email, phone, options) {
    ...
    if (options && options.propertyAddress) {
      backgroundParts.push('Property address: ' + options.propertyAddress);
    }
    if (options && options.concern) {
      backgroundParts.push('Previous listing concern: ' + options.concern);
    }
    ...
    if (viaWebMcp) {
      event.description = 'Expired listing review request submitted via WebMCP agent.';
    }
  ...
  async function submitLead(fields, options) {
    ...
    if (typeof global.posthog !== 'undefined') {
      ...
      global.posthog.capture('lead_submitted', {
        ...
        has_concern_note: !!(fields.concern && fields.concern.length)
      });
    }
